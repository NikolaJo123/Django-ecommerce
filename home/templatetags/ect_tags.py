...
def categoryTree(id,menu,lang):
    ...
    if id <= 0: # Main categories
        if lang == defaultlang: # default language
            query = Categorie.objects.filter(parent_id__isnull=True).order_by("id")
        else: # non default language
            query = Categorie.objects.raw('SELECT c.id,l.title, l.keywords, l.description,l.slug' 
                                      '  FROM product_categorie as c'
                                      '  INNER JOIN product_categorylang as	l'
                                      '  ON c.id = l.category_id'
                                      '  WHERE  parent_id IS NULL and lang=%s ORDER BY c.id',[lang])
        querycount = Categorie.objects.filter(parent_id__isnull=True).count()
    else: # Sub Categories
        if lang == defaultlang: # default language
            query = Categorie.objects.filter(parent_id=id)
        else: # non default language
            query = Categorie.objects.raw('SELECT c.id,l.title, l.keywords, l.description,l.slug'
                                     '  FROM product_categorie as c'
                                     '  INNER JOIN product_categorylang as	l'
        ...
        querycount = Categorie.objects.filter(parent_id= id).count()
    if querycount > 0:
        for rs in query:
            subcount = Categorie.objects.filter(parent_id=rs.id).count()
            if subcount > 0:
